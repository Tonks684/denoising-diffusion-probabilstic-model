...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        assert opt.objective in {'pred_noise', 'pred_x0', 'pred_v'}, 'objective must be either pred_noise (predict noise) or pred_x0 (predict image start) or pred_v (predict v [v-parameterization as defined in appendix D of progressive distillation paper, used in imagen-video successfully])'
    ...
    def ddim_sample(self, shape, return_all_timesteps = False):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            self_cond = x_start if self.self_condition else None
            pred_noise, x_start, *_ = self.model_predictions(img, time_cond, self_cond, clip_x_start = False, rederive_pred_noise = True)
        ...
        final_sample_batch = img
        final_sample_batch = self.unnormalize(final_sample_batch)
        final_sample_batch = self.tensor2img(final_sample_batch)
...
class Trainer(object):
    def __init__(
        ...
        assert has_int_squareroot(opt.num_samples), 'number of samples must have an integer square root'
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        ...
                        iteration_complete = time.time()
                        iteration_time = iteration_complete - iteration_start
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            samples = self.ema.ema_model.sample(batch_size=3)

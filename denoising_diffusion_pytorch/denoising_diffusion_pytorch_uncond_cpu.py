...
Added and embedded functions

# Normalise from 16bit to 0 to 1
def normalize_16bit_image_to_zero_to_one
then perfored at __get_item__ in Dataset class when each img is loaded
# Normalise from 0 to 1 to 16bit
def unnormalize_tensor_to_img
then initalise in Diffusion Model class as self.tensor2img and performed with p_sampling loop and model_predictions

Dataset Class
Dataloader num_workers = 0 for running on cpu
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        dl = DataLoader(
            self.ds, batch_size = train_batch_size, 
